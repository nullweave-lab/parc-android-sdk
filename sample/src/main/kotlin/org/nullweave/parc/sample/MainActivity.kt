...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        clientSecret = field("Client secret", "").apply { inputType = InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_VARIATION_PASSWORD }
        ...
        layout.addView(Button(this).apply { text = "Register device"; setOnClickListener { registerDevice() } })
        layout.addView(Button(this).apply { text = "Run attestation"; setOnClickListener { attest() } })
        layout.addView(output, ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT))
    ...
    private fun credentials(): ClientCredentials = ClientCredentials(clientId.text.toString().trim(), clientSecret.text.toString())
    ...
    private fun registerDevice() = runNetwork {
        ...
        val externalId = Settings.Secure.getString(contentResolver, Settings.Secure.ANDROID_ID) ?: "android-device"
    ...
    private fun runNetwork(block: () -> String) {
        output.text = "Running…"
        Thread {
            val result = runCatching(block).fold(onSuccess = { it }, onFailure = { "Error: ${it.message}" })
